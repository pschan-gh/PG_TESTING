/**
 * MathQuillMatrix v0.10.1, by Han, Jeanine, and Mary
...
if (!jQuery) throw 'MathQuillMatrix requires jQuery 1.5.2+ to be loaded first';
...
/**
 * MathQuillMatrix virtual-DOM tree-node abstract base class
 */
var Node = P(function(_) {
  ...
  _.toString = function() { return '{{ MathQuillMatrix Node #'+this.id+' }}'; };
...
/*********************************************
 * Controller for a MathQuillMatrix instance,
...
/*********************************************************
 * The publicly exposed MathQuillMatrix API.
 ********************************************************/
...
function insistOnInterVer() {
  if (window.console) console.warn(
    'You are using the MathQuillMatrix API without specifying an interface version, ' +
    'which will fail in v1.0.0. Easiest fix is to do the following before ' +
    'doing anything else:\n' +
    '\n' +
    '    MathQuillMatrix = MathQuillMatrix.getInterface(1);\n' +
    '    // now MathQuillMatrix.MathField() works like it used to\n' +
    '\n' +
...
function MathQuillMatrix(el) {
  insistOnInterVer();
  return MQ1(el);
};
MathQuillMatrix.prototype = Progenote.p;
MathQuillMatrix.VERSION = "v0.10.1";
MathQuillMatrix.interfaceVersion = function(v) {
  // shim for #459-era interface versioning (ended with #495)
  if (v !== 1) throw 'Only interface version 1 supported. You specified: ' + v;
  insistOnInterVer = function() {
    if (window.console) console.warn(
      'You called MathQuillMatrix.interfaceVersion(1); to specify the interface ' +
  ...
  insistOnInterVer();
  return MathQuillMatrix;
};
MathQuillMatrix.getInterface = getInterface;
...
function getInterface(v) {
  ...
  var AbstractMathQuillMatrix = APIClasses.AbstractMathQuillMatrix = P(Progenote, function(_) {
  ...
  MQ.prototype = AbstractMathQuillMatrix.prototype;

  APIClasses.EditableField = P(AbstractMathQuillMatrix, function(_, super_) {
  ...
  /**
   * Export the API functions that MathQuillMatrix-ify an HTML element into API objects
   * of each class. If the element had already been MathQuillMatrix-ified but into a
...
MathQuillMatrix.noConflict = function() {
  window.MathQuillMatrix = origMathQuillMatrix;
  return MathQuillMatrix;
};
var origMathQuillMatrix = window.MathQuillMatrix;
window.MathQuillMatrix = MathQuillMatrix;
...
Controller.open(function(_) {
  ...
  _.delegateMouseEvents = function() {
    ...
    this.container.bind('mousedown.mathquill', function(e) {
      ...
      // outside rootjQ, the MathQuillMatrix node corresponding to the target (if any)
...
/*********************************************
 * Manage the MathQuillMatrix instance's textarea
...
var MathCommand = P(MathElement, function(_, super_) {
  ...
  // and selection of the MathQuillMatrix tree, these all take in a direction and
  // the cursor
...
API.StaticMath = function(APIClasses) {
  return P(APIClasses.AbstractMathQuillMatrix, function(_, super_) {
...
var BuiltInOpNames = {}; // the set of operator names like \sin, \cos, etc that
  ...
  // MathQuillMatrix auto-unitalicizes some operator names not in that set, like 'hcf'
  // and 'arsinh', which must be exported as \operatorname{hcf} and
  // \operatorname{arsinh}. Note: over/under line/arrow \lim variants like
  // \varlimsup are not supported
var AutoOpNames = Options.p.autoOperatorNames = { _maxLength: 9 }; // the set
  // of operator names that MathQuillMatrix auto-unitalicizes by default; overridable
...
(function() {
  ...
  // compat with some of the nonstandard LaTeX exported by MathQuillMatrix
...
// and they get copy-pasted into MathQuillMatrix.
//
...
LatexCmds.MathQuillMatrixMathField = P(MathCommand, function(_, super_) {
  _.ctrlSeq = '\\MathQuillMatrixMathField';
  _.htmlTemplate =
...
// From MathQuillMatrix's perspective, it's a Symbol, it can be
...
for (var key in MQ1) (function(key, val) {
  if (typeof val === 'function') {
    MathQuillMatrix[key] = function() {
      insistOnInterVer();
      return val.apply(this, arguments);
    };
    MathQuillMatrix[key].prototype = val.prototype;
  }
  else MathQuillMatrix[key] = val;
}(key, MQ1[key]));
