...
sub new {
	...
	if ($previous eq '') {
		for my $defaultShuffledBucket ( @$default_shuffled_buckets ) {
            # warn main:pretty_print $defaultShuffledBucket->{indices};
			$dnd->addBucket($defaultShuffledBucket->{indices}, label => $defaultShuffledBucket->{label});
		}
	} else {
